...
Description: 汎用簡易予約処理システムです。オンラインで予約を受付けメール転送でお知らせします。PHP Ver.5.3、WordPress Ver.4.0以降で動作させて下さい。
Version: 1.20.1
Author: S.Hayashi
Author URI: http://web.mt-systems.jp
*/
/*  Copyright 2012 - 2015 S.Hayashi
...
*/
/*
 * Updated to 1.20.1 on 2015-01-19
...
class MTS_Simple_Booking
{
	...
	public function init_mtssb()
	{
		...
		// その他設定の読み込み
		$this->miscellaneous = get_option(self::DOMAIN . '_miscellaneous');
		...
		if (is_admin()) {
			...
			if ($this->miscellaneous['adminbar']) {
		...
		// 予約のキャンセル
            ...
            if ($action == 'send') {
                ...
                if ($cancel_url) {
                    // メール送信
                    $mail = $this->_load_module('MTSSB_Mail');
                    if ($mail->cancel_mail($cancel_url)) {
                        $next_url = self::get_permalink_by_slug(self::PAGE_CANCEL_SEND);
                        if (empty($next_url)) {
                            $next_url = add_query_arg(array('action' => 'send'),
                                self::get_permalink_by_slug(MTSSB_Subscription::PAGE_NAME));
                        }
                        wp_redirect($next_url);
                        exit();
                    }
                    // メール送信エラー
                    $subscription->error_send_mail();
                }

                // 予約キャンセル実行(メールのリンクアドレス)
            } elseif (isset($_GET['action']) && $_GET['action'] == 'cancel') {
                if ($subscription->cancel_booking()) {
                    // 予約キャンセル実行のメール送信
                    $mail = $this->_load_module('MTSSB_Mail');
                    if ($mail->remove_mail()) {
                        $next_url = self::get_permalink_by_slug(self::PAGE_CANCEL_THANKS);
                        if (empty($next_url)) {
                            $next_url = add_query_arg(array('action' => 'thanks'),
                                self::get_permalink_by_slug(MTSSB_Subscription::PAGE_NAME));
                        }
                        wp_redirect($next_url);
                        exit();
