...
class MTSSB_Mail
{
	...
	public function contact_mail() {
		...
		// 予約者情報
		$adda = array((empty($contact['name']) ? 'お問い合わせ' : $contact['name']), 0);
    ...
    public function replace_variables($template)
    {
        ...
        // 置き換え追加情報(予約者名、予約ID)
        $adda = array($booking['client']['name'], $this->_make_reserve_id($booking['booking_id'], $booking['booking_time']));

        // 施設情報の埋め込み
        $content = $this->_replace_variable($template, $adda);
	...
	public function cancel_mail($cancel_url)
	{
		...
		// キャンセルURLの埋め込み
		$cancel_content = str_replace('%CANCEL_URL%', $cancel_url, $this->template['cancel_body']);

		// 予約者情報
		$adda = array($booking['client']['name'], 0);

		// 施設情報の埋め込み
		$cancel_content = $this->_replace_variable($cancel_content, $adda);

		$subject = $this->template['cancel_title'];
		$client_to = $booking['client']['email'];
		$mail_ret = true;
		if (!empty($client_to)) {
			$mail_ret = wp_mail($client_to, $subject, $cancel_content, $this->fromshop);
		}
	...
	public function booking_mail() {
        ...
        //date('ymd', $booking['booking_time']) . substr("00{$booking['booking_id']}", -3);
    ...
    private function _send_booking_mail($address, $params)
    {
        ...
        // 本文
        //$body = apply_filters('mtssb_mail_booking_body',
        //    $params['body'],
        //    array('article_id' => $params['article_id'], 'receiver' => $params['receiver'])
        //);
        ...
        // 変数を変換し全文を作る
        $content = $this->_replace_variable($header, $params['adda'])
            . $params['body']
            . $this->_replace_variable($footer, $params['adda']);
        ...
        return wp_mail($address, $subject, $content, $from);
    }
