...
//	protected $ppl_loaded = false;
	 ...
	 * Key
	 */
	private $key = 'MTS_Simple_Booking';
	...
	public function getSessionData($keyname)
	{
		if (isset($this->sess_data[$keyname])) {
			if ($keyname == 'setECResponse' || $keyname == 'doECResponse') {
				//$this->_init_paypal_sdk();
				return unserialize($this->sess_data[$keyname]);
			}
	...
	public function setExpressCheckout()
	{
		...
		if (isset($setECResponse)) {
			...
			if ($setECResponse->Ack == 'Success') {
				...
				// PayPalへのリダイレクト
				$paypalUrl = PPConfigManager::getInstance()->get('service.RedirectURL')
...
<?php
	 ...
	 */
/*
	protected function _init_paypal_sdk($cmd='') {
		if ($this->ppl_loaded) {
			return;
		}

		//$path = dirname(__FILE__) . '/' . self::SDK_PATH;
		...
		// Logging
		if (!empty($cmd)) {
			$this->ppl_logger = new PPLoggingManager($cmd);
		}

		$this->ppl_loaded = true;
